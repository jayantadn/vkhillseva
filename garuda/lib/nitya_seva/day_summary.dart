...
class _DaySummaryState extends State<DaySummary> {
  ...
  Future<void> refresh() async {
    ...
    String dbDate = DateFormat("yyyy-MM-dd").format(widget.date);
    List sessionsList = await FB().getList(
    ...
    for (Session session in sessions) {
      String dbSession = session.timestamp.toIso8601String().replaceAll(
        ".",
        "^",
      );
          ...
          for (Session session in sessions) {
            ...
            for (var amount in Const().nityaSeva['amounts'] as List) {
              ...
              if (index >= 0) {
                ...
                _amountTableTicketRow[_amountTableTicketRow.length - 1].add(
                  "0",
                );
              }
            }
            ...
            for (var ticket in tickets) {
              ...
              Map<String, dynamic> ticketJson = Map<String, dynamic>.from(
                ticket,
              );
  ...
  Future<void> _shareDaySummary(BuildContext context) async {
    ...
    if (image != null) {
      ...
      Share.shareXFiles([XFile(imagePath.path)], text: msg);
    }
  }
    ...
    return Container(
      ...
      child: Table(
        ...
        children: [
          ...
          TableRow(
            children:
                _amountTableHeaderRow.map((header) {
                  return Padding(
          ...
          ..._amountTableTotalRow.map((row) {
            return TableRow(
              decoration:
                  _amountTableTotalRow.indexOf(row) == 0
                      ? const BoxDecoration(
