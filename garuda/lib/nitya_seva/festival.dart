...
class _FestivalRecordState extends State<FestivalRecord> {
  ...
  Future<void> refresh() async {
    ...
    await _lock.synchronized(() async {
      ...
      for (var dateRaw in datesRaw) {
        ...
        dateMap.forEach((key, valueRaw) {
          ...
          Map<String, dynamic> sessionJson = Map<String, dynamic>.from(
            sessionMap['Settings'],
          );
          ...
          if (session.name != "Nitya Seva" && session.name != "Testing") {
            Map<String, dynamic> festival = {
              ...
              "sessions": [
                {'settings': session, "numTickets": 0, "sumAmount": 0},
              ],
            };
    ...
    _festivals.sort(
      (a, b) => a['sessions'][0]['settings'].timestamp.compareTo(
        b['sessions'][0]['settings'].timestamp,
      ),
    );
            ...
            Row(
              children: [
                ...
                if (festival['icon'].isNotEmpty)
                  CircleAvatar(backgroundImage: AssetImage(festival['icon'])),
