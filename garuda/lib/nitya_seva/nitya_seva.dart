...
import 'package:garuda/nitya_seva/festival.dart';
//TODO import 'package:garuda/nitya_seva/laddu/laddu.dart';
import 'package:garuda/nitya_seva/session.dart';
import 'package:garuda/nitya_seva/ticket_page.dart';
import 'package:garuda/widgets/common_widgets.dart';
import 'package:garuda/nitya_seva/day_summary.dart';
import 'package:garuda/widgets/date_header.dart';
import 'package:garuda/widgets/launcher_tile.dart';
import 'package:garuda/home/settings.dart';
...
class _NityaSevaState extends State<NityaSeva> {
  ...
  Future<void> refresh({bool spinner = true}) async {
    ...
    _sessions.clear();
    String dbDate = DateFormat('yyyy-MM-dd').format(_selectedDate);
    List<dynamic> sessions = await FB().getList(
  ...
  void _postValidation(Session session) {
    ...
    refresh(spinner: false).then((_) async {
      ...
      if (errors.isNotEmpty) {
        String? ret = await CommonWidgets().createErrorDialog(
          context: context,
          errors: errors,
          post: true,
        );
        if (ret == 'Edit') {
          ...
          // delete in server
          FB().deleteValue(
            path: "${Const().dbrootGaruda}/NityaSeva/$dbDate/${session.name}",
          );
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        DropdownButtonFormField<String>(
                          ...
                          items:
                              _sevaList.map((FestivalSettings value) {
                            ...
                            Expanded(
                              child: ElevatedButton(
                                ...
                                onPressed: () async {
                                  ...
                                  if (session == null) {
                                    List<String> errors = _preValidation(
                                      newSession,
                                    );
                                    String? ret = 'Proceed';
                                    if (errors.isNotEmpty) {
                          ...
                          callback: LauncherTileCallback(
                            onClick: () {
                              Navigator.push(
                                ...
                                MaterialPageRoute(
                                  builder:
                                      (context) => TicketPage(session: session),
                                ),
                              );
                            },
                          ),
