  ...
  Future<void> _populateTable() async {
    ...
    for (int? amount in pushpanjaliTickets) {
      ...
      if (listFiltered.isNotEmpty) {
        Ticket entry = listFiltered.reduce(
          (currentLowest, next) =>
  ...
  Color _getColorBasedOnNumber(String header) {
    ...
    for (var amount in amounts) {
      if (amount.keys.first == number.toString()) {
        Map<String, dynamic> amountJson = Map<String, dynamic>.from(
          amount.values.first,
        );
                ...
                itemBuilder: (BuildContext context, int index) {
                  ...
                  return Container(
                    margin: const EdgeInsets.symmetric(
                      horizontal: 8.0,
                      vertical: 4.0,
                    ), // Adjust margin to remove top gap
