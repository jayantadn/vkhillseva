...
import 'package:garuda/nitya_seva/session_summary.dart';
import 'package:garuda/nitya_seva/tally_cash.dart';
import 'package:garuda/nitya_seva/tally_upi_card.dart';
...
class _TicketPageState extends State<TicketPage> {
  ...
  initState() {
    ...
    String sessionKey = widget.session.timestamp.toIso8601String().replaceAll(
      ".",
      "^",
    );
    FB().listenForChange(
  ...
  Future<void> refresh({bool? spinner = true}) async {
    ...
    // fetch tickets
    String dbDate = DateFormat("yyyy-MM-dd").format(widget.session.timestamp);
    String dbSession = widget.session.timestamp.toIso8601String().replaceAll(
      ".",
      "^",
    );
  ...
  Future<List<String>> _postvalidateTicket() async {
    ...
    if (sessionsList.isEmpty) {
      ...
      List<Session> sessions = [];
      for (var sessionRaw in sessionsList) {
        Map<String, dynamic> sessionMap = Map<String, dynamic>.from(
          sessionRaw['Settings'],
        );
