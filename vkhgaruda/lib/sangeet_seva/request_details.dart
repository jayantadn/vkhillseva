...
class _RequestDetailsState extends State<RequestDetails> {
  ...
  Future<void> _performAction(String action) async {
    if (widget.pendingRequest == null) {
      ...
      if (action == "Approve") {
        ...
        _notifyUser(widget.eventRecord.status);
      }
    } else {
  ...
  Future<void> _notifyUser(String status) async {
    // prepare notification
    String date =
        DateFormat("EEE, dd MM, yyyy").format(widget.eventRecord.date);
    String startTime =
        DateFormat("hh:mm a").format(widget.eventRecord.eventStart);
    String endTime = DateFormat("hh:mm a").format(widget.eventRecord.eventEnd);
    NotificationEntry notif = NotificationEntry(
        title: status == "Approve"
            ? "Request approved"
            : (status == "Reject" ? "Request rejected" : status),
        timestamp: DateTime.now(),
        isRead: false,
        message: "Event: $date [$startTime - $endTime]",
        actions: status == "Approve" ? ["advisory"] : []);

    // send notification to user
    if (_eventRequester?.fcmToken != null &&
        _eventRequester!.fcmToken.isNotEmpty) {
      Utils().sendNotification(
          fcmToken: _eventRequester!.fcmToken,
          title: notif.title,
          body: notif.message);
    }
    ...
    String dbpathNotif =
        "${Const().dbrootSangeetSeva}/Notifications/Performers/${_eventRequester!.mobile}/$dbkey";
    await FB().setJson(path: dbpathNotif, json: notif.toJson());
  }
                          ...
                          if (widget.eventRecord.notePerformer.isNotEmpty)
                            Widgets().createTopLevelCard(
                              ...
                              child: Align(
                                ...
                                child: Text(widget.eventRecord.notePerformer),
                              ),
                            ),
                        ]),
