...
class FB {
  ...
  Future<void> listenForChange(String path, FBCallbacks callbacks) async {
    ...
    // Get initial snapshot first to know what already exists
    final initialSnapshot = await dbRef.get();
    final Set<String> initialKeys = {};

    if (initialSnapshot.exists && initialSnapshot.value != null) {
      final data = initialSnapshot.value as Map<dynamic, dynamic>;
      initialKeys.addAll(data.keys.map((k) => k.toString()));
    }

    var listener = dbRef.onChildAdded.listen((event) {
      // Only trigger callback for children added AFTER initial load
      if (!initialKeys.contains(event.snapshot.key)) {
  ...
  Future<void> listenForChangeInList(String path, FBCallbacks callbacks) async {
    final dbRef = FirebaseDatabase.instance.ref(path);
    ...
    // Get initial snapshot first to know what already exists in the list
    final initialSnapshot = await dbRef.get();
    final Set<String> initialKeys = {};

    if (initialSnapshot.exists && initialSnapshot.value != null) {
      if (initialSnapshot.value is List) {
        final data = List<dynamic>.from(initialSnapshot.value as List);
        for (int index = 0; index < data.length; index++) {
          if (data[index] != null) {
            initialKeys.add(index.toString());
          }
        }
      }
    }
    ...
    var listener = dbRef.onChildAdded.listen((event) {
      // Only trigger callback for items added AFTER initial load
      if (!initialKeys.contains(event.snapshot.key)) {
        callbacks.add(event.snapshot.value);
      }
    });
