...
class Utils {
  ...
  Future<void> checkForNewVersion(BuildContext context, String app) async {
    ...
    await rc.setConfigSettings(
      RemoteConfigSettings(
        ...
        minimumFetchInterval: const Duration(
          seconds: 0,
        ), // keep for production: Duration(hours: 6)
      ),
    );

    await rc.setDefaults({
      "${app}_trigger_update": false,
      "${app}_version": "0.0.0",
      "${app}_version_suffix": 0,
    });
    await rc.fetchAndActivate();

    final info = await PackageInfo.fromPlatform();
    int curver = int.parse(info.buildNumber);
    String newverStr = rc.getString("${app}_version_suffix");
    int newver = int.tryParse(newverStr) ?? 0;
    if (newver > curver) {
      String newverName = rc.getString("${app}_version");

      String msg = "New version available: $newverName";
      if (kIsWeb) {
        msg += "\n\nKindly press the browser refresh button to update";
      } else {
        msg += "\n\nPlease contact admin for new version";
      }

      Widgets().showMessage(context, msg);
    }
  }
  ...
  DateTime convertDbKeyToTimestamp(String dbkey) {
    String datetimeElem = dbkey.replaceAll("^", ".");
    return DateTime.parse(datetimeElem);
  }
  ...
  Color? getNityaSevaAmountColor(String? amount) {
    // Default to a light grey background if amount/color not found
    final Color? defaultColor = Colors.grey[200];
    if (amount == null) return defaultColor;

    final List amounts = Const().nityaSeva['amounts'] ?? [];
  ...
  Future<void> sendNotification({
    ...
    final response = await http.post(
      Uri.parse("https://sendnotification-26lx7cfwtq-uc.a.run.app"),
  ...
  Future<void> sendNotificationToTopic({
    ...
    final response = await http.post(
      Uri.parse("https://sendtopicnotification-26lx7cfwtq-uc.a.run.app"),
      headers: {'Content-Type': 'application/json'},
      body: jsonEncode({
        'topic': topic,
        'title': title,
  ...
  String convertTimestampToDbKey(DateTime timestamp) {
    return timestamp.toIso8601String().replaceAll(".", "^");
  }
