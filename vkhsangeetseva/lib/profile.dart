...
class _ProfileState extends State<Profile> {
  ...
  Future<void> refresh() async {
      ...
      if (basics != null) {
        ...
        if (exists) {
          ...
          if (_userDetailsOld != null &&
              (_userDetailsOld!.fcmToken.isEmpty ||
                  _userDetailsOld!.fcmToken != (widget.fcmToken ?? ""))) {
            String dbpath =
                "${Const().dbrootSangeetSeva}/Users/$mobile/fcmToken";
            FB().setValue(
                path: dbpath, value: widget.fcmToken ?? ""); // no need to await
          }
        }
      }
    ...
    await _lock.synchronized(() async {
      ...
      // hack: facilitate browse button always for audio clips
      if (_audioClips.isEmpty || _audioClips.last.isNotEmpty) {
        _audioClips.add("");
      }
  ...
  Widget _createAudioClipTile(int index) {
    String title = _getFileNameFromUrl(_audioClips[index]);

    return Column(
      children: [
        if (index == 0 || _audioClips[index - 1].isNotEmpty)
          Row(
            children: [
              Expanded(
                child: Text(
              ...
              if (title == "No files selected")
                OutlinedButton(
                  ...
                  onPressed: () async {
                    ...
                    String url = await FS()
                        .uploadBytes(dstPath: dstPath, bytes: file.bytes!);

                    setState(() {
                      _audioClips[index] = url;

                      // hack for the browse button not to disappear
                      if (_audioClips.last.isNotEmpty) {
                        _audioClips.add("");
                      }
              ...
              if (title != "No files selected")
                IconButton(
                  ...
                  onPressed: () {
                    if (_audioClips[index].isNotEmpty) {
                      launchUrl(Uri.parse(_audioClips[index]));
                    }
                  },
                ),
                ...
                IconButton(
                  ...
                  onPressed: () async {
                    Widgets().showConfirmDialog(
                        context,
                        "Are you sure to delete the audio clip?",
                        "Delete", () async {
                      String path = FS().getPathFromUrl(_audioClips[index]);
                      await FS().deleteFile(filePath: path);

                      setState(() {
                        _audioClips.removeAt(index);
                      });
        ...
        if (index < _audioClips.length - 1) SizedBox(height: 10),
      ],
    );
  }
  ...
  Widget _createSangeetSadhanaTile(int index) {
    return ListTile(
      ...
      trailing: IconButton(
        ...
        onPressed: () async {
          Widgets().showConfirmDialog(
              context, "Are you sure to delete?", "Delete", () async {
            setState(() {
              _exp.removeAt(index);
            });
          ...
          content: SingleChildScrollView(
            child: Column(
              ...
              children: [
                ...
                Text(
                    'Name: ${userDetails.salutation == "Others" ? "" : userDetails.salutation} ${userDetails.name}'),
  ...
  Future<void> _save({bool goback = true}) async {
    // validation of form
                        ...
                        child: Padding(
                          ...
                          child: Column(
                            children: [
                              ...
                              // button for adding youtube link
                              SizedBox(height: 20),
                              TextButton(
                                  onPressed: () async {
                              ...
                              ...List.generate(_audioClips.length, (index) {
                                return _createAudioClipTile(index);
                              }),
                            ],
                          ),
                        ),
